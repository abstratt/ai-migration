...
- **Core migration churn is larger for g94 (144 lines) than g951 (98 lines)**, across more files (30 vs 19). The two runs' task 06 differed: **g94's task 06 applied real mechanical Cat-A rewrites** to the Java/Groovy build logic the scanner could resolve (`GradleJavadocsPlugin.java` — `setTitle`/`setEncoding`/`setDocEncoding`/`setCharSet`/`setHeader`/`setClasspath`/`setDestinationDir` → lazy accessors; `PerformanceReporter.groovy` — `spec.debug =` → `getDebug().set(...)`), whereas **g951's task 06 produced only `MIGRATION_NOTES.md`** (no source changes). In **both** runs, task 06 made **no changes to the Kotlin build logic** (`.kt`/`.gradle.kts`): the scanner's documented Kotlin type-resolution blind spot means its confirmed hits there were false positives (a property *name* matched against an unrelated Gradle class while the real receiver was `java.io.File`, the docs plugin's own model types, japicmp/javassist, non-lazy `Project`/`Task` accessors, etc.) and the genuine Kotlin Provider-API breakages were invisible to it. Those breakages surfaced instead as `help`/`assemble` compile errors and were fixed by hand in tasks 07/08.
