...
1. **Summary**: The repository, its migration status (migrated, skipped, failed), the local branch name, the **start time**, **end time**, and **total wall-clock duration** of the migration (derive start from the first commit on the migration branch and end from the current time or last commit — use ISO 8601 with timezone, e.g. `2026-04-15T15:24:00-03:00`), and the **Assistant identification** — the same three-part trailer used on commit messages, i.e. `<<Tool Name>> / <<Friendly Model Name>> / <<model-id>>` (e.g. `Claude Code / Claude Opus 4.6 / claude-opus-4-6`). Use `Unknown Tool`, `Unknown Model`, or `unknown-id` for any part that cannot be determined. This line must be present in every report.
