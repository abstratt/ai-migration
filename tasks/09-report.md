...
3. **`MIGRATION_NOTES.md` characterization (only if the file exists on the branch).** In brute-force mode (build-script migration skipped) the file is expected to be absent — omit this section entirely; its absence is not a defect. Otherwise, describe the file by what it *actually is*, not by what task 06 aspires to produce. Before writing this section, run the two audits from task 06 step 5(b) against the file on disk and include their exact output: the **(b.1) canonical-boilerplate count** (must be 0) and the **(b.2) coverage check** (must report `OK`, i.e. every scanner-flagged file is documented), plus how many residual scanner hits/files the file documents and whether reasons are grouped under root-cause headings or just raw categorical labels. Do not use words like "curated", "analyzed", or "per-entry reasoning" unless both audits pass and the reasons genuinely name the construct at each site. If the file is mostly machine-generated categorical reasons, say so plainly (e.g. *"596 entries across 7 categorical reason strings; 511 of 596 share the same Cat-B label — a backlog of deferred sites, not per-site analysis"*). Misdescribing `MIGRATION_NOTES.md` in the REPORT is a self-reporting failure — the file is the only audit trail of what task 06 considered, and its character must be reported truthfully.
